        ...
        public static void Run()
        {
            ...
            for (var i = 0; i < threads.Length; i++)
            {
                ...
                threads[i] = new Thread(
                    ...
                    {
                        var msg = messages[i0];
                        Console.WriteLine($"Начало обработки сообщения {msg} в потоке {Thread.CurrentThread.ManagedThreadId}");
                        handle.WaitOne();

                        lock(messages_list)
                            messages_list.Add(messages[i0].Length);

                        Console.WriteLine($"Обработка сообщения {msg} в потоке {Thread.CurrentThread.ManagedThreadId} завершено");
